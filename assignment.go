...
func (_ AssignSelf) Unassigned(d *Daemon, state VniState, leaderState LeaderState, vni uint64) error {
	return d.db.NewVniUpdate(vni).Revision(state.Revision).LeaderState(leaderState).Type(FailoverDecided).Current("", NoLease).Next(d.Config.Node, TempLease).Run()
}
...
func (_ AssignOther) Unassigned(d *Daemon, state VniState, leaderState LeaderState, vni uint64) error {
	...
	return d.db.NewVniUpdate(vni).Revision(state.Revision).LeaderState(leaderState).Type(FailoverDecided).Current("", NoLease).Next(node, TempLease).Run()
}
...
func (_ AssignOther) Periodical(d *Daemon, state VniState, leaderState LeaderState, vni uint64) error {
	...
	if state.Type == Idle && state.Current == d.Config.Node {
		return d.db.NewVniUpdate(vni).Revision(state.Revision).LeaderState(leaderState).Type(MigrationDecided).Next(node, TempLease).Run()
	}
	return nil
}
